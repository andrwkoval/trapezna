...
class Motion:
    ...
    def __init__(self):
        ...
        self.inname = 'assets/sample_2.avi'
    ...
    def prep(self):
        ...
        self.accumulator = np.zeros(
            (height, width), np.float32)  # 32 bit accumulator

    def run(self):
        cap = cv2.VideoCapture(self.inname)
        fgbg = cv2.createBackgroundSubtractorMOG2(
            varThreshold=80, detectShadows=False)
        while(1):
            ret, frame = cap.read()
            if not ret:
                break
            ...
            cv2.imshow("frame", self.accumulator)

    def write(self):

        self.ab = cv2.convertScaleAbs(
            255-np.array(self.accumulator, 'uint8'))

        # only get reasonable high values, above mean
        ret, self.acc_thresh = cv2.threshold(
            self.ab, self.ab.mean(), 255, cv2.THRESH_TOZERO)
        ...
        backg = cv2.addWeighted(
            np.array(acc_col, "uint8"), 0.45, self.orig_image, 0.55, 0)

        cv2.imwrite(
            str(self.outname + "/heatmap_background.jpg"), backg)
